...
This table is autogenerated by [`{file}`] using [`cargo-bloat`]:

| Crate | Version | Size | Maintained | TOML version |
|:-----:|:-------:|:----:|:----------:|:------------:|
{
  "\n".join(
    f"| [{result["name"]}] | {result["version"]} | {result['size']} | {result["maintained"]} | {result["toml-ver"]} |"
...
{
  "\n".join(
    set(f"[{result["name"]}]: {result["url"]}" for result in results)
  )
}
